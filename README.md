...
HNUST ACM 在线评测系统（[acm.hnust.edu.cn](http://acm.hnust.edu.cn)）的辅助工具集。
...
# → OCR 服务运行在 http://127.0.0.1:8765
```

然后在 v5 脚本面板中：
- 勾选 **"使用本地OCR"**
- OCR Server 地址保持默认 `http://127.0.0.1:8765`
...
| Contest | 题目 | 内容 |
|---------|------|------|
| 3761 | 4 道 | 直接插入排序、希尔排序、快速排序、简单选择排序 |
| 3762 | 3 道 | 堆排序、归并排序、基数排序 |

---

## ⚠️ 注意事项

- **不要将 `session.pkl`、验证码图片、含真实账号信息的脚本上传到 GitHub！**
- `.gitignore` 已配置排除敏感文件，上传前请确认配置区使用占位符
